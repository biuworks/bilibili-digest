...
var BILI_LEARNING_STORE = (() => {
  ...
  function buildBackup({ notes, learning, exportedAt, analysisSystemPrompt } = {}) {
    const backup = {
      kind: BACKUP_KIND,
      schemaVersion: SCHEMA_VERSION,
    ...
    // 仅在确实有自定义时写入；旧备份无此字段保持兼容。
    const custom =
      typeof analysisSystemPrompt === "string" ? analysisSystemPrompt.trim() : "";
    if (custom) backup.analysisSystemPrompt = custom;
    return backup;
  }
