...
var BILI_LEARNING_STORE = (() => {
  ...
  async function saveLearningRecord(
    ...
    if (!repository) throw new Error("保存概览需要学习仓储");
  ...
  function buildBackup({ notes, learning, exportedAt, analysisSystemPrompt, customSystemPrompts } = {}) {
    const backup = {
      kind: BACKUP_KIND,
      schemaVersion: SCHEMA_VERSION,
    ...
    // 仅写入非空自定义；旧备份无字段保持兼容。
    const map = {};
    if (customSystemPrompts && typeof customSystemPrompts === "object") {
      for (const [key, value] of Object.entries(customSystemPrompts)) {
        if (typeof value === "string" && value.trim()) map[key] = value.trim();
      }
    }
    if (!map["analysis.md"] && typeof analysisSystemPrompt === "string" && analysisSystemPrompt.trim()) {
      map["analysis.md"] = analysisSystemPrompt.trim();
    }
    if (Object.keys(map).length) backup.customSystemPrompts = map;
    return backup;
  }
  ...
  function parseBackup(payload) {
    ...
    if (
      payload.analysisSystemPrompt != null &&
      typeof payload.analysisSystemPrompt !== "string"
    ) {
      return {
        ok: false,
        error: "INVALID_BACKUP",
        message: "备份里的概览系统提示词格式不正确。",
      };
    }
    if (
      payload.customSystemPrompts != null &&
      (typeof payload.customSystemPrompts !== "object" ||
        Array.isArray(payload.customSystemPrompts))
    ) {
      return {
        ok: false,
        error: "INVALID_BACKUP",
        message: "备份里的自定义系统提示词格式不正确。",
      };
    }
