  ...
  function createNotesService({
    ...
    async function exportLearningBackup() {
      ...
      if (typeof getSettings === "function") {
        try {
          ...
          customSystemPrompts = settings?.customSystemPrompts || {};
        } catch (_) {
          customSystemPrompts = {};
        }
      }
      return {
        success: true,
        backup: LEARNING_STORE.buildBackup({ notes, learning, customSystemPrompts }),
      };
    }
    ...
    async function importLearningBackup(payload) {
      ...
      return {
        ...
        // 交给 background 合并进 settings；空/缺省由 merge 规则忽略。
        customSystemPrompts: parsed.backup.customSystemPrompts,
        analysisSystemPrompt: parsed.backup.analysisSystemPrompt,
      };
    }
