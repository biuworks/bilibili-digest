...
var BILI_NOTES_SERVICE = (() => {
  ...
  function createNotesService({
    ...
    async function importLearningBackup(payload) {
      ...
      const merged = LEARNING_STORE.mergeBackup(notes, learning, parsed.backup);
      // 笔记与概览各自单事务写入 IndexedDB。中途失败重跑导入即可：
      // 合并按 updatedAt 取新，天然幂等。
      if (merged.learning.length) {
        await repositories.learning().commit({ put: merged.learning });
      }
      await repositories.notes().commit({ put: merged.notes });
