  ...
  function createQaService({
    ...
    async function askQuestion({
      ...
      try {
        ...
        const { valid } = CITATIONS.validateCitations({
        ...
        } else if (!valid.length && parsed?.citations?.length !== 0) {
          // 模型给了引用但全被两道闸拦下：视为没有依据。
          answer = FALLBACK_ANSWER;
        }
        ...
        return {
          ...
          clickable: [...CITATIONS.clickableTimestamps(entry.answer, context.timeRange)],
        };
      } catch (error) {
