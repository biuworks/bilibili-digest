...
var BILI_QA_CITATIONS = (() => {
  ...
  // 成对包裹引号的完整码位集：弯引号、直引号、全角、低引号、
  // 直角引号、书名号——模型用哪种包回答都剥得掉。
  const QUOTE_EDGE_CHARS = "\\u0022\\u0027\\u201C\\u201D\\u2018\\u2019\\u201E\\u201F\\u00AB\\u00BB\\u2039\\u203A\\u300C\\u300D\\u300E\\u300F\\uFF02\\uFF07\\u300A\\u300B";
  const EDGE_QUOTE_RUN = new RegExp(
    `^[${QUOTE_EDGE_CHARS}\\s]+|[${QUOTE_EDGE_CHARS}\\s]+$`,
    "g",
  );

  /** 剥掉文本首尾的成对包裹引号（含空白）。保存与渲染两处共用。 */
  function stripWrappingQuotes(text) {
    return String(text || "")
      .trim()
      .replace(EDGE_QUOTE_RUN, "");
  }
