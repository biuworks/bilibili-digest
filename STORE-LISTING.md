...
```text
dist/digest-for-bilibili-0.4.4.zip
```
...
界面正文对比度默认值有微调，六张已用当前构建重截（`store/screenshots/01`–`06`），上传时全部替换。Edge 徽标仍用 `store/logo-300.png`，图标仍用 `icons/icon128.png`。
...
| --- | --- |
| 安装包 | 上传上面的 zip |
| 详细说明 | 「版本历史」清单顶部加 0.4.4 一行（已写进下面「详细说明」） |
| 版本说明 | 贴下面「给审核员的变更摘要」 |
| 截图 | 六张全部替换（正文对比度默认值变了） |
| 名称、简短说明、类别、单一用途 | 不用改 |
| 权限理由 | 不用改（权限集合没变） |
...
```text
0.4.4 为功能更新版本，Chrome 与 Edge 使用同一份 MV3 安装包。

- 设置页新增外观设置：五套主题色板、明暗模式（跟随系统 / 浅色 / 深色）与三档文字浓度
- 各主题色板按明暗两种模式校准文字对比度，正文可读性不随换色下降
- 播放页上的扩展按钮跟随所选主题色，设置变更即时生效
- 界面字号与外观设置即改即存，无需重新授权

权限、主机范围和数据类别与 0.4.3 相同。
```
...
• 界面外观 — 五套主题色板、明暗模式（跟随系统 / 浅色 / 深色）与三档文字浓度，加上 80%–160% 的界面字号，即改即生效，无需重新授权。各色板的文字对比度均按明暗两种模式校准。
...
- [ ] 数据披露与 `PRIVACY.md` 一致。
- [ ] 远程代码选择“否”。
- [ ] 升级路径真机验证：0.4.3 造好笔记 / 概览 / 字幕缓存后覆盖安装
      0.4.4，三类数据原样可用（IndexedDB 自动迁移，无需手动操作）。
- [ ] 外观：切换主题色 / 明暗模式 / 文字浓度即时生效，重启面板后保持；
      播放页按钮跟随主题色，设置变更无需刷新页面。
