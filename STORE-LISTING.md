...
| 商店 | 版本 | 上线日期 | 链接 |
| --- | --- | --- | --- |
| Chrome Web Store | 0.4.2 提审中（在线 0.4.0） | 0.1.0 2026-08-15 / 0.2.0 2026-08-22 | https://chromewebstore.google.com/detail/digest-for-bilibili/cfndfabkpfgihcgknbgfnkjlmndhhmfc |
| Microsoft Edge Add-ons | 0.4.2 提审中（在线 0.4.0） | 0.1.0 2026-08-17 / 0.2.0 2026-08-22 | https://microsoftedge.microsoft.com/addons/detail/digest-for-bilibili/jlfmjhkcbnkgghefieaagkcccjojmnkm |

两个商店用**同一个 zip**。0.4.2 相对商店在线的 0.4.0，包含两块内容：一是
0.4.1 未随商店发布的**字幕轨切换**；二是本版的四处缺陷修复与体验打磨——
概览生成期间顺句/翻译结果不再被覆盖回滚，首句字幕之前的笔记不再错取视频
结尾内容，问答生成中可随时点「停止」取消，修复问答偶发误报「未找到」的
检索缺陷，消除非编辑区域闪烁的文本插入符。
...
Chrome 开发者后台与 Edge 合作伙伴中心上传同一份。相对在线的 0.4.0，本版包含
0.4.1 未发布的字幕轨切换（新功能）与本版的缺陷修复。
...
| --- | --- |
| 安装包 | 上传上面的 zip |
| 详细说明 | 「版本历史」清单顶部加 0.4.2 与 0.4.1 两行；「字幕阅读」功能条目补上字幕轨切换（均已写进下面「详细说明」） |
| 版本说明 | 贴下面「给审核员的变更摘要」 |
| 截图 | 不用改 |
| 名称、简短说明、类别、单一用途 | 不用改 |
| 权限理由 | 不用改（权限集合没变） |
...
```text
0.4.2 相对商店里的 0.4.0：新增字幕轨切换与若干缺陷修复，
Chrome 与 Edge 使用同一份 MV3 安装包。

- 多字幕轨的视频可在侧边栏切换字幕轨，并标明每条轨道的语言与来源
  （人工 / AI）；切换后顺句、翻译与概览按所选轨道重新生成
...
- [ ] 数据披露与 `PRIVACY.md` 一致。
- [ ] 远程代码选择“否”。
- [ ] 升级路径真机验证：0.4.0 造好笔记 / 概览 / 字幕缓存后覆盖安装
