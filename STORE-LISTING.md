...
| 商店 | 版本 | 上线日期 | 链接 |
| --- | --- | --- | --- |
| Chrome Web Store | 0.4.3 提审中（在线 0.4.2） | 0.1.0 2026-08-15 / 0.2.0 2026-08-22 | https://chromewebstore.google.com/detail/digest-for-bilibili/cfndfabkpfgihcgknbgfnkjlmndhhmfc |
| Microsoft Edge Add-ons | 0.4.3 提审中（在线 0.4.2） | 0.1.0 2026-08-17 / 0.2.0 2026-08-22 | https://microsoftedge.microsoft.com/addons/detail/digest-for-bilibili/jlfmjhkcbnkgghefieaagkcccjojmnkm |

两个商店用**同一个 zip**。0.4.3 相对商店在线的 0.4.2 全部是缺陷修复与体验打磨：
问答与笔记的交互状态缺陷（提问期间切视频串台、后台任务冲掉编辑草稿、按钮
状态卡死）、推理类模型兼容性、概览重试与截断恢复、导出与备份细节、设置页
输入体验等。
...
```text
dist/digest-for-bilibili-0.4.3.zip
```
...
| --- | --- |
| 安装包 | 上传上面的 zip |
| 详细说明 | 「版本历史」清单顶部加 0.4.3 一行（已写进下面「详细说明」） |
| 版本说明 | 贴下面「给审核员的变更摘要」 |
| 截图 | 不用改 |
| 名称、简短说明、类别、单一用途 | 不用改 |
| 权限理由 | 不用改（权限集合没变） |
...
```text
0.4.3 为缺陷修复版本，Chrome 与 Edge 使用同一份 MV3 安装包。

- 提升对推理类模型的兼容性，此前部分此类模型的请求必然失败
- 修复提问期间切换视频后，问答历史与回答可能显示到另一个视频名下的问题
- 后台任务完成时不再冲掉正在编辑的笔记草稿
- 修复部分场景下「保存中…」等按钮状态永久卡住的问题
- 模型响应在数组中间被截断时也能自动修复，概览与问答更稳定
- 概览失败分块重试不再误删相邻分块边界处的章节
- 超长视频的问答引用不再失效
- 同一时刻已有笔记时如实提示，不再假报保存成功
- 界面字号输入、模型列表获取超时等多处设置页体验修复
- 本地数据库瞬时故障后自动重连

权限、主机范围和数据类别与 0.4.2 相同。
```
...
功能
• 字幕阅读 — 字幕取自 B 站官方接口，支持原文 / 译文 / 双语三种视图。多字幕轨的视频可切换轨道，并标明每条轨道的语言与来源（人工 / AI）。当前句跟随播放高亮，点任意一句跳到视频对应时间点，支持全文搜索、一键复制和导出 TXT。
...
版本 0.4.3 — 修复提问期间切视频问答串台、笔记草稿被后台任务冲掉、按钮状态卡死等一批交互缺陷；提升推理类模型兼容性；概览重试与截断恢复更可靠；多处导出与备份细节修复
版本 0.4.2 — 概览生成期间顺句与翻译结果不再被覆盖回滚；修复视频开头记录笔记错取视频结尾的缺陷；问答生成中可随时停止；修复长视频问答偶发误报「未找到」的缺陷；消除非编辑区域闪烁的文本插入符
版本 0.4.1 — 多字幕轨视频可切换字幕轨，并标明每条的语言与人工 / AI 来源；切换后顺句、翻译与概览按新轨道隔离
...
- [ ] `npm test` 全部通过。
- [ ] `npm run package` 得到 `dist/digest-for-bilibili-0.4.3.zip`。
...
- [ ] 数据披露与 `PRIVACY.md` 一致。
- [ ] 远程代码选择“否”。
- [ ] 升级路径真机验证：0.4.2 造好笔记 / 概览 / 字幕缓存后覆盖安装
      0.4.3，三类数据原样可用（IndexedDB 自动迁移，无需手动操作）。
- [ ] 多字幕轨视频：切换轨道后字幕 / 顺句 / 概览按新轨加载，旧轨缓存不串。
