...
笔记按视频与分 P 保存；可只看当前分 P，或翻全部历史。卡片上会标出 UP 主，翻全部时还会带视频标题。当前列表可导出为带时间戳链接的 Markdown，范围跟「本视频 / 全部」一致，也可按正文、标题或 UP 主搜索。概览或本视频笔记里还可以导出学习稿：YAML 头、章节、金句、笔记，可选附带当前视图的字幕。设置页可导出或恢复 JSON 备份（不含 API 密钥），也可把界面字号调大。
